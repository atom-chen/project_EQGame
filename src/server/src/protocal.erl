...
encode(#npc_map_mapping_info{id=ID, npc_id=NPC_ID, npc_name=NPC_NAME, born_id=BORN_ID, script_id=SCRIPT_ID, action=ACTION, npc_key=NPC_KEY, towards=TOWARDS}) ->
	{?msg_npc_map_mapping_info, <<ID:?INT, NPC_ID:?INT, (net_helper:encode_string(NPC_NAME))/binary, BORN_ID:?INT, SCRIPT_ID:?INT, ACTION:?INT, (net_helper:encode_string(NPC_KEY))/binary, TOWARDS:?INT>>};
...
encode(#player_task_action{type=TYPE, finish_amount=FINISH_AMOUNT}) ->
	{?msg_player_task_action, <<TYPE:?INT, FINISH_AMOUNT:?INT>>};
encode(#player_task{id=ID, account=ACCOUNT, task_id=TASK_ID, step_index=STEP_INDEX, track=TRACK, status=STATUS, create_date=CREATE_DATE, action_list=ACTION_LIST}) ->
	{?msg_player_task, <<ID:?UINT64, (net_helper:encode_string(ACCOUNT))/binary, TASK_ID:?INT, STEP_INDEX:?INT, TRACK:?INT, STATUS:?INT, (net_helper:get_encode_binary_data(encode(CREATE_DATE)))/binary, (net_helper:encode_list(fun(E)-> net_helper:get_encode_binary_data(encode(E)) end, ACTION_LIST))/binary>>};
...
encode(#req_enter_common_scene{scene_id=SCENE_ID, copy_id=COPY_ID}) ->
	{?msg_req_enter_common_scene, <<SCENE_ID:?INT, COPY_ID:?INT>>};
encode(#notify_enter_common_scene{scene_id=SCENE_ID, born_id=BORN_ID}) ->
	{?msg_notify_enter_common_scene, <<SCENE_ID:?INT, BORN_ID:?INT>>};
encode(#notify_enter_player_house{account=ACCOUNT, born_id=BORN_ID, house_tplt_id=HOUSE_TPLT_ID}) ->
	{?msg_notify_enter_player_house, <<(net_helper:encode_string(ACCOUNT))/binary, BORN_ID:?INT, HOUSE_TPLT_ID:?INT>>};
encode(#req_teleport{id=ID, copy_id=COPY_ID}) ->
	{?msg_req_teleport, <<ID:?INT, COPY_ID:?INT>>};
...
encode(#req_walk_for_body_action{curr_pos=CURR_POS, dest_pos=DEST_POS, my_action=MY_ACTION, you_action=YOU_ACTION, you_account=YOU_ACCOUNT, you_pos=YOU_POS, move_type=MOVE_TYPE}) ->
	{?msg_req_walk_for_body_action, <<(net_helper:get_encode_binary_data(encode(CURR_POS)))/binary, (net_helper:get_encode_binary_data(encode(DEST_POS)))/binary, (net_helper:encode_string(MY_ACTION))/binary, (net_helper:encode_string(YOU_ACTION))/binary, (net_helper:encode_string(YOU_ACCOUNT))/binary, (net_helper:get_encode_binary_data(encode(YOU_POS)))/binary, MOVE_TYPE:?INT>>};
...
encode(#req_chat_big_expression{exp_id=EXP_ID}) ->
	{?msg_req_chat_big_expression, <<EXP_ID:?INT>>};
encode(#notify_chat_big_expression{account=ACCOUNT, player_name=PLAYER_NAME, exp_id=EXP_ID}) ->
	{?msg_notify_chat_big_expression, <<(net_helper:encode_string(ACCOUNT))/binary, (net_helper:encode_string(PLAYER_NAME))/binary, EXP_ID:?INT>>};
...
encode(#notify_open_all_light{turn_on=TURN_ON}) ->
	{?msg_notify_open_all_light, <<TURN_ON:?INT>>};
...
encode(#notify_agree_invitation{agreer=AGREER, type=TYPE}) ->
	{?msg_notify_agree_invitation, <<(net_helper:encode_string(AGREER))/binary, TYPE:?INT>>};
...
encode(#notify_update_task_progress{id=ID, action_type=ACTION_TYPE, action_amount=ACTION_AMOUNT}) ->
	{?msg_notify_update_task_progress, <<ID:?UINT64, ACTION_TYPE:?INT, ACTION_AMOUNT:?INT>>};
...
encode(#notify_buy_new_house_success{}) ->
	{?msg_notify_buy_new_house_success, null};
...
encode(#notify_today_flower_egg_count{house_owner=HOUSE_OWNER, egg_count=EGG_COUNT, flower_count=FLOWER_COUNT}) ->
	{?msg_notify_today_flower_egg_count, <<(net_helper:encode_string(HOUSE_OWNER))/binary, EGG_COUNT:?INT, FLOWER_COUNT:?INT>>};
...
encode(#req_common_scene_state{teleport_id=TELEPORT_ID}) ->
	{?msg_req_common_scene_state, <<TELEPORT_ID:?INT>>};
encode(#scene_state{scene_id=SCENE_ID, copy_id=COPY_ID, count=COUNT, copy_name=COPY_NAME, max_players=MAX_PLAYERS, busy_players=BUSY_PLAYERS, loose_players=LOOSE_PLAYERS, status=STATUS}) ->
	{?msg_scene_state, <<SCENE_ID:?INT, COPY_ID:?INT, COUNT:?INT, (net_helper:encode_string(COPY_NAME))/binary, MAX_PLAYERS:?INT, BUSY_PLAYERS:?INT, LOOSE_PLAYERS:?INT, STATUS:?INT>>};
encode(#notify_common_scene_state{scene_list=SCENE_LIST}) ->
	{?msg_notify_common_scene_state, <<(net_helper:encode_list(fun(E)-> net_helper:get_encode_binary_data(encode(E)) end, SCENE_LIST))/binary>>};
encode(#req_cupid_participate{account=ACCOUNT, sex=SEX, tags=TAGS}) ->
	{?msg_req_cupid_participate, <<(net_helper:encode_string(ACCOUNT))/binary, SEX:?INT, (net_helper:encode_list(fun(E)-> net_helper:encode_string(E) end, TAGS))/binary>>};
encode(#req_cupid_cancel_participate{account=ACCOUNT}) ->
	{?msg_req_cupid_cancel_participate, <<(net_helper:encode_string(ACCOUNT))/binary>>};
encode(#notify_cupid_participate{result=RESULT}) ->
	{?msg_notify_cupid_participate, <<RESULT:?INT>>};
encode(#notify_cupid_cancel_participate{result=RESULT}) ->
	{?msg_notify_cupid_cancel_participate, <<RESULT:?INT>>};
encode(#notify_cupid_girl_match_result{tags=TAGS}) ->
	{?msg_notify_cupid_girl_match_result, <<(net_helper:encode_list(fun(E)-> net_helper:encode_string(E) end, TAGS))/binary>>};
encode(#notify_cupid_boy_match_result{tags=TAGS, girl_result=GIRL_RESULT}) ->
	{?msg_notify_cupid_boy_match_result, <<(net_helper:encode_list(fun(E)-> net_helper:encode_string(E) end, TAGS))/binary, GIRL_RESULT:?INT>>};
encode(#req_cupid_girl_confirm{account=ACCOUNT, result=RESULT}) ->
	{?msg_req_cupid_girl_confirm, <<(net_helper:encode_string(ACCOUNT))/binary, RESULT:?INT>>};
encode(#req_cupid_boy_confirm{account=ACCOUNT, result=RESULT, girl_result=GIRL_RESULT}) ->
	{?msg_req_cupid_boy_confirm, <<(net_helper:encode_string(ACCOUNT))/binary, RESULT:?INT, GIRL_RESULT:?INT>>};
encode(#req_create_party{type=TYPE, title=TITLE, description=DESCRIPTION}) ->
	{?msg_req_create_party, <<TYPE:?INT, (net_helper:encode_string(TITLE))/binary, (net_helper:encode_string(DESCRIPTION))/binary>>};
encode(#notify_create_party_result{result=RESULT}) ->
	{?msg_notify_create_party_result, <<RESULT:?INT>>};
encode(#req_edit_party{type=TYPE, title=TITLE, description=DESCRIPTION}) ->
	{?msg_req_edit_party, <<TYPE:?INT, (net_helper:encode_string(TITLE))/binary, (net_helper:encode_string(DESCRIPTION))/binary>>};
encode(#notify_edit_party_result{result=RESULT}) ->
	{?msg_notify_edit_party_result, <<RESULT:?INT>>};
encode(#req_delete_party{}) ->
	{?msg_req_delete_party, null};
encode(#req_get_party_list{}) ->
	{?msg_req_get_party_list, null};
encode(#party_data{create_account=CREATE_ACCOUNT, type=TYPE, title=TITLE, desc=DESC}) ->
	{?msg_party_data, <<(net_helper:encode_string(CREATE_ACCOUNT))/binary, TYPE:?INT, (net_helper:encode_string(TITLE))/binary, (net_helper:encode_string(DESC))/binary>>};
encode(#notify_party_list{partys=PARTYS}) ->
	{?msg_notify_party_list, <<(net_helper:encode_list(fun(E)-> net_helper:get_encode_binary_data(encode(E)) end, PARTYS))/binary>>}.
...
decode(?msg_npc_map_mapping_info, Binary) ->
	net_helper:decode([int, int, string, int, int, int, string, int], Binary, [npc_map_mapping_info]);
...
decode(?msg_player_task_action, Binary) ->
	net_helper:decode([int, int], Binary, [player_task_action]);
decode(?msg_player_task, Binary) ->
	net_helper:decode([uint64, string, int, int, int, int, {user_define, fun(Bin)->decode(?msg_stime, Bin)end}, {array, user_define, fun(Bin)->decode(?msg_player_task_action, Bin)end}], Binary, [player_task]);
...
decode(?msg_notify_enter_common_scene, Binary) ->
	net_helper:decode([int, int], Binary, [notify_enter_common_scene]);
decode(?msg_notify_enter_player_house, Binary) ->
	net_helper:decode([string, int, int], Binary, [notify_enter_player_house]);
decode(?msg_req_teleport, Binary) ->
	net_helper:decode([int, int], Binary, [req_teleport]);
...
decode(?msg_req_walk_for_body_action, Binary) ->
	net_helper:decode([{user_define, fun(Bin)->decode(?msg_point, Bin)end}, {user_define, fun(Bin)->decode(?msg_point, Bin)end}, string, string, string, {user_define, fun(Bin)->decode(?msg_point, Bin)end}, int], Binary, [req_walk_for_body_action]);
...
decode(?msg_req_chat_big_expression, Binary) ->
	net_helper:decode([int], Binary, [req_chat_big_expression]);
decode(?msg_notify_chat_big_expression, Binary) ->
	net_helper:decode([string, string, int], Binary, [notify_chat_big_expression]);
...
decode(?msg_notify_open_all_light, Binary) ->
	net_helper:decode([int], Binary, [notify_open_all_light]);
...
decode(?msg_notify_agree_invitation, Binary) ->
	net_helper:decode([string, int], Binary, [notify_agree_invitation]);
...
decode(?msg_notify_update_task_progress, Binary) ->
	net_helper:decode([uint64, int, int], Binary, [notify_update_task_progress]);
...
decode(?msg_notify_buy_new_house_success, Binary) ->
	net_helper:decode([], Binary, [notify_buy_new_house_success]);
...
decode(?msg_notify_today_flower_egg_count, Binary) ->
	net_helper:decode([string, int, int], Binary, [notify_today_flower_egg_count]);
...
decode(?msg_req_common_scene_state, Binary) ->
	net_helper:decode([int], Binary, [req_common_scene_state]);
decode(?msg_scene_state, Binary) ->
	net_helper:decode([int, int, int, string, int, int, int, int], Binary, [scene_state]);
decode(?msg_notify_common_scene_state, Binary) ->
	net_helper:decode([{array, user_define, fun(Bin)->decode(?msg_scene_state, Bin)end}], Binary, [notify_common_scene_state]);
decode(?msg_req_cupid_participate, Binary) ->
	net_helper:decode([string, int, {array, string}], Binary, [req_cupid_participate]);
decode(?msg_req_cupid_cancel_participate, Binary) ->
	net_helper:decode([string], Binary, [req_cupid_cancel_participate]);
decode(?msg_notify_cupid_participate, Binary) ->
	net_helper:decode([int], Binary, [notify_cupid_participate]);
decode(?msg_notify_cupid_cancel_participate, Binary) ->
	net_helper:decode([int], Binary, [notify_cupid_cancel_participate]);
decode(?msg_notify_cupid_girl_match_result, Binary) ->
	net_helper:decode([{array, string}], Binary, [notify_cupid_girl_match_result]);
decode(?msg_notify_cupid_boy_match_result, Binary) ->
	net_helper:decode([{array, string}, int], Binary, [notify_cupid_boy_match_result]);
decode(?msg_req_cupid_girl_confirm, Binary) ->
	net_helper:decode([string, int], Binary, [req_cupid_girl_confirm]);
decode(?msg_req_cupid_boy_confirm, Binary) ->
	net_helper:decode([string, int, int], Binary, [req_cupid_boy_confirm]);
decode(?msg_req_create_party, Binary) ->
	net_helper:decode([int, string, string], Binary, [req_create_party]);
...
decode(?msg_req_edit_party, Binary) ->
	net_helper:decode([int, string, string], Binary, [req_edit_party]);
...
decode(?msg_req_delete_party, Binary) ->
	net_helper:decode([], Binary, [req_delete_party]);
decode(?msg_req_get_party_list, Binary) ->
	net_helper:decode([], Binary, [req_get_party_list]);
decode(?msg_party_data, Binary) ->
	net_helper:decode([string, int, string, string], Binary, [party_data]);
decode(?msg_notify_party_list, Binary) ->
	net_helper:decode([{array, user_define, fun(Bin)->decode(?msg_party_data, Bin)end}], Binary, [notify_party_list]).
