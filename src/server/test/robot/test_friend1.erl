...
start() ->
    player_friends:test(),
    start(["127.0.0.1", "2222", "friend1@35info.cn"]).
...
init() ->
    [{?msg_notify_login_result, fun process_notify_login_result/4},
     ...
     {?msg_notify_chat_tell, fun process_notify_chat_tell/4},
...
%%--------------------------------------------------------------------------------
process_notify_chat_tell(#notify_chat_tell{}, Socket, _List, 
			   #state{account=Account}=State) ->
    io:format("10. friend1 听到 friend2说话后，回家，同时邀请对方进入自已的房间\n"),
    send_data(Socket, #req_enter_player_house{account=Account}),

    send_data(Socket, #req_invite_someone{invitee="friend2", type=?ivt_enter_house}),
    
    State.    

process_notify_player_enter_scene(#notify_player_enter_scene{account=Friend2}, Socket, _List, 
			   #state{account=Account}=State) ->

    case Friend2 /= Account of
	true ->
	    io:format(" 11. friend1 看到friend2，直接将其踢回家(看到有人就踢)\n"),
	    send_data(Socket, #req_kick_guest{target_player=Friend2});
	false -> 
	    ok
    end,
