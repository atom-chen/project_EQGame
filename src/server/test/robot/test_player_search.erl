...
%%
start() ->
    start(["127.0.0.1", "2222", "hongjx@35info.cn"]).
...
process_notify_ver_validate({notify_ver_validate, ?ver_ok}, Socket, _List, #state{account=RoleName}=State) ->
    Data = #req_login{account=RoleName, password="Aa35info"},
    send_data(Socket, Data),
    State;
...
process_notify_enter_scene_result({notify_enter_scene_result, Result}, Socket, _List, State) ->
    case Result of
        ?sr_ok ->
	    send_data(Socket, #req_search_player_by_nick{nick="mawj@35info.cn"}),

	    send_data(Socket, #req_search_player_by_mail{mail="hongjx@35info.cn"}),
