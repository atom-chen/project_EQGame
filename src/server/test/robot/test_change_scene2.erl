...
-record(state, {bag, money,account}).
...
process_notify_enter_scene_result({notify_enter_scene_result, Result}, Socket, _List, 
				  #state{account=RoleName}=State) ->
    case Result of
        ?sr_ok ->
	    io:format(" 进入公共场景\n"),
	    ...
	    io:format(" 开始一段行走\n"),
	    send_data(Socket, #req_start_walk{curr_pos=#point{x=3693, z=9306}, dest_pos=#point{x=3693 + 80, z=9306 + 80}}),
	    timer:sleep(100),
	    io:format("回家\n"),
	    send_data(Socket, #req_enter_player_house{account=RoleName}),
	    ok;
        ?sr_fail ->
