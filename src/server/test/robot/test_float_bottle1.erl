...
%%
start() ->
    start(["127.0.0.1", "2222", "bottle_droper"]).
...
process_notify_ver_validate({notify_ver_validate, ?ver_ok}, Socket, _List,
    ...
    Data = #req_login{account=RoleName, password=""},
    send_data(Socket, Data),
    State;
...
process_notify_enter_scene_result({notify_enter_scene_result, Result}, Socket, _List, State) ->
    case Result of
	    ...
	    timer:sleep(1000),
            ...
            send_data(Socket,  #req_drop_bottle{msg="bottle 1 girl visible true", 
				    type=?bt_make_friend, 
				    sex=?st_girl, 
				    account_visible=?TRUE}), 
	    
            send_data(Socket,  #req_drop_bottle{msg="bottle 2 girl visible false", 
				    type=?bt_make_friend, 
				    sex=?st_girl, 
				    account_visible=?FALSE}), 

            send_data(Socket,  #req_drop_bottle{msg="bottle 3 girl visible false", 
				    type=?bt_common, 
				    sex=?st_girl, 
				    account_visible=?FALSE}), 

            send_data(Socket,  #req_drop_bottle{msg="bottle 4 girl visible false", 
				    type=?bt_common, 
				    sex=?st_girl, 
				    account_visible=?FALSE}), 

            send_data(Socket,  #req_drop_bottle{msg="bottle 5 girl visible false", 
				    type=?bt_common, 
				    sex=?st_girl, 
				    account_visible=?FALSE}), 


            printf("~n~nreq_all_bottle_info~n"),
	    send_data(Socket, #req_all_bottle_info{}),


	    %%send_data(Socket, #req_del_bottle_log{is_pick_type=?FALSE, index=0}),
