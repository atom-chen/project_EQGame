...
start() ->
    player_friends:test(),
    start(["127.0.0.1", "2222", "change_look1"]).
...
process_notify_enter_scene_result(_Msg, Socket, _List, 
    ...
    timer:sleep(100),
    io:format("请求开始美容\n"),
    send_data(Socket, #req_start_change_looks{}),

    timer:sleep(100),
    io:format("取消美容\n"),
    send_data(Socket, #req_cancel_change_looks{}),
    ...
    timer:sleep(100),
    io:format("请求结束美容, 将发型改为第五种\n"),
    send_data(Socket, #req_end_change_looks{new_hair=5, 
					   new_hair_color=1,
					   new_face=1,
					   new_skin_color=1,
					   new_beard=1}),
    ...
    timer:sleep(100),
    io:format("请求结束美容, 将发型改为第6种\n"),
    send_data(Socket, #req_end_change_looks{new_hair=6,
					   new_hair_color=1,
					   new_face=1,
					   new_skin_color=1,
					   new_beard=1}),
