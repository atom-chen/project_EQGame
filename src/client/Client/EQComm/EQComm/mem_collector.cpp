...
mc_malloc(mc_collector_t **mc, size_t size)
{
	...
	mem_block = (mc_collector_t *)malloc(sizeof(mc_collector_t) + size);
	if (mem_block == NULL){
		return NULL;
	}
	...
	*mc = mem_block;
	return (unsigned char *) mem_block + sizeof(mc_collector_t);
}
...
mc_free(void *data)
{
	...
	mem_block = (mc_collector_t *) ((unsigned char *) data - sizeof(mc_collector_t));
