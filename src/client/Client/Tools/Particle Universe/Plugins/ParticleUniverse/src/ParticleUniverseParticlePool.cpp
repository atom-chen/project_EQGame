...
namespace ParticleUniverse
{
	...
	void ParticlePool::destroyAllVisualParticles(void)
	{
		...
		{
			OGRE_DELETE_T(*it, VisualParticle, Ogre::MEMCATEGORY_SCENE_OBJECTS);
		}
	...
	void ParticlePool::destroyAllSystemParticles(void)
	{
		...
		std::vector<Ogre::String>::iterator itSystem;
	...
	//-----------------------------------------------------------------------
	void ParticlePool::_increaseVisualParticlePool(size_t size, std::vector<ParticleBehaviour*>& behaviours)
	{
		size_t oldSize = mVisualParticlesPool.getSize();
		if (size < oldSize)
			return;
		...
		{
			particle = OGRE_NEW_T(VisualParticle, Ogre::MEMCATEGORY_SCENE_OBJECTS)();
	...
	//-----------------------------------------------------------------------
	void ParticlePool::_increaseParticleEmitterPool(size_t size, std::vector<ParticleBehaviour*>& behaviours, ParticleTechnique* technique)
	{
		size_t oldSize = mParticleEmitterPool.getSize();
		if (size < oldSize)
			return;
		...
		size_t increment = size / numberOfEmittedEmitters;
	...
	//-----------------------------------------------------------------------
	void ParticlePool::_increaseParticleTechniquePool(size_t size, std::vector<ParticleBehaviour*>& behaviours, ParticleSystem* system)
	{
		size_t oldSize = mParticleTechniquePool.getSize();
		if (size < oldSize)
			return;
		...
		// Distribute size equally
		size_t increment = size / numberOfEmittedTechniques;
	...
	//-----------------------------------------------------------------------
	void ParticlePool::_increaseParticleAffectorPool(size_t size, std::vector<ParticleBehaviour*>& behaviours, ParticleTechnique* technique)
	{
		size_t oldSize = mParticleAffectorPool.getSize();
		if (size < oldSize)
			return;
		...
		// Distribute size equally
		size_t increment = size / numberOfEmittedAffectors;
	...
	//-----------------------------------------------------------------------
	void ParticlePool::_increaseParticleSystemPool(size_t size, std::vector<ParticleBehaviour*>& behaviours, ParticleTechnique* technique)
	{
		size_t oldSize = mParticleSystemPool.getSize();
		if (size < oldSize)
			return;
		...
		// Distribute size equally
		size_t increment = size / numberOfEmittedSystems;
				...
				if (system)
				{
					...
					for (size_t i = 0; i < increment; i++)
					{
						clonedSystem = ParticleSystemManager::getSingletonPtr()->createParticleSystem(
							emitter->getEmitsName()+ ss.str() + Ogre::StringConverter::toString(i), 
	...
	//-----------------------------------------------------------------------
	Particle* ParticlePool::releaseParticle (const Particle::ParticleType particleType, const Ogre::String& name)
	{
		switch(particleType)
		{
			case Particle::PT_VISUAL:
