...
namespace ParticleUniverse
{
	...
	const Ogre::Real DoScaleEventHandler::DEFAULT_SCALE_FRACTION = 0.2f;
	...
	//-----------------------------------------------------------------------
	const Ogre::Real DoScaleEventHandler::getScaleFraction(void) const
	{
		return mScaleFraction;
	}
	//-----------------------------------------------------------------------
	void DoScaleEventHandler::setScaleFraction(const Ogre::Real scaleFraction)
	{
		mScaleFraction = scaleFraction;
	}
	//-----------------------------------------------------------------------
	void DoScaleEventHandler::_handle (ParticleTechnique* particleTechnique, Particle* particle, Ogre::Real timeElapsed)
	{
		if (!particle)
			return;
