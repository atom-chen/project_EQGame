...
{
	// Constants
	const Ogre::Real SphereSurfaceEmitter::DEFAULT_RADIUS = 10.0f;
	...
	//-----------------------------------------------------------------------
	const Ogre::Real SphereSurfaceEmitter::getRadius(void) const
	{
		return mRadius;
	}
	//-----------------------------------------------------------------------
	void SphereSurfaceEmitter::setRadius(const Ogre::Real radius)
	{
		mRadius = radius;
	}
	//-----------------------------------------------------------------------
	void SphereSurfaceEmitter::_initParticlePosition(Particle* particle)
	{
		...
		mRandomVector = Ogre::Vector3(Ogre::Math::RangeRandom(-1, 1), 
		...
		if (sys)
		{
			particle->position = getDerivedPosition() + sys->getDerivedOrientation() * mRandomVector * mRadius;
		}
		else
		{
