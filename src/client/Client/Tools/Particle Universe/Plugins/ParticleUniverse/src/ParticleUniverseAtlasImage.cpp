...
namespace ParticleUniverse
{
	...
	bool AtlasImage::addImage (const Ogre::Image* image)
	{
		...
		mImageList.push_back(OGRE_NEW Ogre::Image(*image));
		if (mAlwaysUpdate)
		{
			_compile();
		}
	...
	//-----------------------------------------------------------------------
	void AtlasImage::save (const Ogre::String& filename)
	{
		if (mAtlas)
		{
	...
	void AtlasImage::_compile (void)
	{
		...
		size_t imageColums = Ogre::Math::Ceil(Ogre::Math::Sqrt((Ogre::Real)mImageList.size()));
		size_t imageRows = Ogre::Math::Ceil((Ogre::Real)(mImageList.size()) / (Ogre::Real)imageColums);
