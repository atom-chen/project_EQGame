...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	SphereSet::SphereSet(const Ogre::String& name, unsigned int poolSize, bool externalData) :
    ...
    //-----------------------------------------------------------------------
	Sphere* SphereSet::createSphere(const Ogre::Vector3& position)
    {
		...
		// Merge
        Ogre::Vector3 vecAdjust(mDefaultRadius, mDefaultRadius, mDefaultRadius);
	...
	//-----------------------------------------------------------------------
	Sphere* SphereSet::createSphere(Ogre::Real x, Ogre::Real y, Ogre::Real z)
	{
		return createSphere(Ogre::Vector3( x, y, z ));
	}
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	void SphereSet::setDefaultRadius(Ogre::Real radius)
	{
		mDefaultRadius = radius;
	}
	//-----------------------------------------------------------------------
	Ogre::Real SphereSet::getDefaultRadius(void) const
	{
	...
	//-----------------------------------------------------------------------
	void SphereSet::setBounds(const Ogre::AxisAlignedBox& box, Ogre::Real radius)
	{
		mAABB = box;
	...
	void SphereSet::_updateRenderQueue(Ogre::RenderQueue* queue)
    {
		if (!mExternalData)
		{
			...
			ActiveSphereList::iterator it;
			for(it = mActiveSpheres.begin(); it != mActiveSpheres.end(); ++it)
			{
				injectSphere(*(*it));
			}
			endSpheres();
	...
	//-----------------------------------------------------------------------
	bool SphereSet::sphereVisible(Ogre::Camera* cam, const Sphere& sphere)
	{
	...
	void SphereSet::_genVertices(Sphere& sphere)
	{
		...
		Ogre::RGBA* pCol;
		Ogre::Real radius = getDefaultRadius();
		if (sphere.mOwnRadius)
		{
			radius = sphere.getOwnRadius();
		}

		Ogre::Real stepRingAngle = (Ogre::Math::PI / mNumberOfRings);
		Ogre::Real stepSegmentAngle = (2 * Ogre::Math::PI / mNumberOfSegments);

		for(unsigned int ring = 0; ring <= mNumberOfRings; ring++)
		{
			Ogre::Real r0 = radius * sinf (ring * stepRingAngle);
			Ogre::Real y0 = radius * cosf (ring * stepRingAngle);

			for(unsigned int segment = 0; segment <= mNumberOfSegments; segment++)
			{
				Ogre::Real x0 = r0 * sinf(segment * stepSegmentAngle);
				Ogre::Real z0 = r0 * cosf(segment * stepSegmentAngle);

				// Vertex
				Ogre::Vector3 orientedVertex = sphere.mOrientation * Ogre::Vector3(x0, y0, z0);
				...
				*mLockPtr++ = (float) (orientedVertex.z + sphere.mPosition.z);
				...
				// Normal
				Ogre::Vector3 vNormal = orientedVertex.normalisedCopy();
	...
	//-----------------------------------------------------------------------
	const Ogre::String& SphereSet::getMovableType(void) const
	{
		return SphereSetFactory::PU_FACTORY_TYPE_NAME;
	}
	//-----------------------------------------------------------------------
	Ogre::String SphereSetFactory::PU_FACTORY_TYPE_NAME = "SphereSet";
	//-----------------------------------------------------------------------
	const Ogre::String& SphereSetFactory::getType(void) const
	{
		return PU_FACTORY_TYPE_NAME;
	}
	//-----------------------------------------------------------------------
	Ogre::MovableObject* SphereSetFactory::createInstanceImpl(const Ogre::String& name, const Ogre::NameValuePairList* params)
	{
		...
		if (poolSize > 0)
		{
			return OGRE_NEW SphereSet(name, poolSize, externalData);
		}
		else
		{
			return OGRE_NEW SphereSet(name);
		}
	}
	//-----------------------------------------------------------------------
