...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	const Ogre::String& VertexEmitter::getMeshName(void) const
	{
		return mMeshName;
	}
	//-----------------------------------------------------------------------
	void VertexEmitter::setMeshName(const Ogre::String& meshName)
	{
	...
	//-----------------------------------------------------------------------
	unsigned short VertexEmitter::_calculateRequestedParticles(Ogre::Real timeElapsed)
	{
	...
	//-----------------------------------------------------------------------
	void VertexEmitter::_preProcessParticles(ParticleTechnique* technique, Ogre::Real timeElapsed)
	{
	...
	void VertexEmitter::_generatePoints (void)
	{
		...
		if (!mMesh && mMeshName != Ogre::StringUtil::BLANK)
		{
			_loadMesh();
		}
	...
	void VertexEmitter::_generatePointsFromVertexBuffer(void)
	{
		if (mMaxVertexCount > 1)
		{
			...
			Ogre::Vector3 v1(pReal[0], pReal[1], pReal[2]);
			mVertexBuffer += mStep * mVertexSize; // No validation; assume that it doesn't exceed.
			mPositionElement->baseVertexPointerToElement(mVertexBuffer, &pReal);
			Ogre::Vector3 v2(pReal[0], pReal[1], pReal[2]);
			_generatePoints (v1, v2);
			mVertexCount += mStep;
		}
	}
	//-----------------------------------------------------------------------
	void VertexEmitter::_generatePoints (const Ogre::Vector3& startVector,
		const Ogre::Vector3& endVector,
		Ogre::Real lengthIncrement)
	{
		...
		@remarks
			Use localspace for the vectors and only transform as soon as a particle is emitted.
		*/
		std::list<Ogre::Vector3>::iterator iterator = mSpawnPositionList.end();
		Ogre::Vector3 start = startVector;
		if (lengthIncrement > 0.0)
		{
			// Determine number of dividers, based on the length increment
			mSegments = ((endVector - startVector).length()) / lengthIncrement - 1;
		}
        Ogre::Vector3 increment = (endVector - startVector) / mSegments;
