...
namespace ParticleUniverse
{
	...
	{
		// Create Box set
		mBoxSet = OGRE_NEW BoxSet("", 0, true);
	...
	//-----------------------------------------------------------------------
	void BoxRenderer::_setMaterialName(const Ogre::String& materialName)
	{
		mBoxSet->setMaterialName(materialName);
	}
	//-----------------------------------------------------------------------
	void BoxRenderer::_notifyCurrentCamera(Ogre::Camera* cam)
	{
	...
	//-----------------------------------------------------------------------
	void BoxRenderer::_notifyDefaultDimensions(Ogre::Real width, Ogre::Real height, Ogre::Real depth)
	{
	...
	//-----------------------------------------------------------------------
	void BoxRenderer::setRenderQueueGroup(Ogre::uint8 queueId)
	{
		mQueueId = queueId;
		if (mBoxSet)
		{
