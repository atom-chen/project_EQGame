...
{
	// Constants
	const Ogre::Vector3 MeshSurfaceEmitter::DEFAULT_SCALE = Ogre::Vector3::UNIT_SCALE;
	...
	inline void Triangle::calculateSquareSurface (void)
	{
		...
		*/
		...
		Ogre::Real a = sqrt (	(v1.x - v3.x) * (v1.x - v3.x) + 
							(v1.y - v3.y) * (v1.y - v3.y) + 
							(v1.z - v3.z) * (v1.z - v3.z));
		Ogre::Real b = sqrt (	(v2.x - v1.x) * (v2.x - v1.x) + 
							(v2.y - v1.y) * (v2.y - v1.y) + 
							(v2.z - v1.z) * (v2.z - v1.z));
		Ogre::Real c = sqrt (	(v3.x - v2.x) * (v3.x - v2.x) + 
							(v3.y - v2.y) * (v3.y - v2.y) + 
							(v3.z - v2.z) * (v3.z - v2.z));
		Ogre::Real p = 0.5 * (a + b + c);
	...
	//-----------------------------------------------------------------------
	const Ogre::Vector3 Triangle::getRandomTrianglePosition (void)
	{
		...
		Ogre::Real a = Ogre::Math::RangeRandom(0, 1);
	...
	{
		Ogre::Real mult = Ogre::Math::RangeRandom(0, 1);
		Ogre::Real randomVal = Ogre::Math::RangeRandom(0, 3);
		PositionAndNormal pAndN;
		pAndN.position = Ogre::Vector3::ZERO;
		pAndN.normal = Ogre::Vector3::ZERO;
		if (randomVal < 1)
		{
			pAndN.position = Ogre::Vector3(	v2.x + mult*(v1.x - v2.x),
											v2.y + mult*(v1.y - v2.y),
											v2.z + mult*(v1.z - v2.z));
			pAndN.normal = en1;
		}
		else
		{
			if (randomVal < 2)
			{
				pAndN.position = Ogre::Vector3(	v3.x + mult*(v2.x - v3.x),
												v3.y + mult*(v2.y - v3.y),
												v3.z + mult*(v2.z - v3.z));
				pAndN.normal = en2;
			}
			else
			{
				...
				pAndN.position = Ogre::Vector3(	v1.x + mult*(v3.x - v1.x),
												v1.y + mult*(v3.y - v1.y),
												v1.z + mult*(v3.z - v1.z));
				pAndN.normal = en3;
			}
	...
	//-----------------------------------------------------------------------
	//-----------------------------------------------------------------------
	MeshInfo::MeshInfo (const Ogre::String& meshName, 
		...
		mDistribution(distribution)
	{
		...
		getMeshInformation(mesh, Ogre::Vector3::ZERO, orientation, scale);
	}
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	inline Ogre::Real MeshInfo::getGaussianRandom (Ogre::Real high, Ogre::Real cutoff)
	{
		Ogre::Real x1 = 0;
		Ogre::Real x2 = 0;
		Ogre::Real w = 0;
		...
		do
		{
			x1 = Ogre::Math::RangeRandom(0,1);
			x2 = Ogre::Math::RangeRandom(0,1);
			w = x1 * x1 + x2 * x2;
	...
	const size_t MeshInfo::getRandomTriangleIndex (void)
	{
		...
		{
			index = getGaussianRandom(_triangles.size() - 1);
		}
		else
			index = Ogre::Math::RangeRandom(0, _triangles.size() - 1);
		...
		for ( unsigned short i = 0; i < mesh->getNumSubMeshes(); ++i)
		{
			...
			if((!submesh->useSharedVertices)||(submesh->useSharedVertices && !added_shared))
			{
				...
				for( size_t j = 0; j < vertex_data->vertexCount; ++j, vertex += vbuf->getVertexSize())
				{
					...
					Ogre::Vector3 nt(pReal[0], pReal[1], pReal[2]);
    ...
    {
		if (mMeshInfo)
			OGRE_DELETE_T(mMeshInfo, MeshInfo, Ogre::MEMCATEGORY_SCENE_OBJECTS);
	}
	//-----------------------------------------------------------------------
	void MeshSurfaceEmitter::_prepare(ParticleTechnique* particleTechnique)
    {
		// Build the data
		if (mMeshName != Ogre::StringUtil::BLANK)
		{
			build();
		}
	}
	...
	void MeshSurfaceEmitter::_initParticlePosition(Particle* particle)
	{
		...
		if (mMeshInfo)
		{
			...
			if (mAutoDirection)
			{
				if (pAndN.normal != Ogre::Vector3::ZERO)
				{
					...
					Ogre::Radian angle(mDynamicAttributeHelper.calculate(mDynAngle, mParentTechnique->getParentSystem()->getTimeElapsedSinceStart()));
    ...
    //-----------------------------------------------------------------------
	const Ogre::String& MeshSurfaceEmitter::getMeshName(void) const
	{
		return mMeshName;
	}
	//-----------------------------------------------------------------------
	void MeshSurfaceEmitter::setMeshName(const Ogre::String& meshName, bool doBuild)
	{
		mMeshName = meshName;
	...
	//-----------------------------------------------------------------------
	const Ogre::Vector3& MeshSurfaceEmitter::getScale (void) const
	{
		return mScale;
	}
	//-----------------------------------------------------------------------
	void MeshSurfaceEmitter::setScale (const Ogre::Vector3& scale)
	{
		mScale = scale;
	}
    //-----------------------------------------------------------------------
	void MeshSurfaceEmitter::build(void)
	{
		...
		// Generate meshinfo.
		mMeshInfo = OGRE_NEW_T(MeshInfo, Ogre::MEMCATEGORY_SCENE_OBJECTS)(mMeshName, mDistribution, mOrientation, mScale);
	}
	//-----------------------------------------------------------------------
