...
{
	// Constants
	const Ogre::Real InterParticleCollider::DEFAULT_ADJUSTMENT = 1.0f;
	...
	//-----------------------------------------------------------------------
	Ogre::Real InterParticleCollider::getAdjustment(void) const
	{
		return mAdjustment;
	}
	//-----------------------------------------------------------------------
	void InterParticleCollider::setAdjustment(Ogre::Real adjustment)
	{
	...
	{
		// Disactivate spatial hashing
		particleTechnique->setSpatialHashing(true);
	}
	//-----------------------------------------------------------------------
	void InterParticleCollider::_affect(ParticleTechnique* particleTechnique, Particle* particle, Ogre::Real timeElapsed)
	{
		...
		if (particle->particleType != Particle::PT_VISUAL && 
			...
			particle->direction == Ogre::Vector3::ZERO)
		{
			return;
		}
	...
	//-----------------------------------------------------------------------
	bool InterParticleCollider::_validateAndExecuteSphereCollision (Particle* particle1, Particle* particle2, Ogre::Real timeElapsed)
	{
		...
		if ((vp1->position - vp2->position).length() < mAdjustment * (vp1->radius + vp2->radius))
		{
			...
			*/
			Ogre::Vector3 n = vp1->position - vp2->position;
			n.normalise();
			switch(mInterParticleCollisionResponse)
			{
				case IPCR_AVERAGE_VELOCITY:
				{
					...
					Ogre::Real velocity1 = vp1->direction.length();
					Ogre::Real velocity2 = vp2->direction.length();
					Ogre::Real averageVelocity = 0.5 * (velocity1 + velocity2);
				...
				case IPCR_ANGLE_BASED_VELOCITY:
				{
					...
					Ogre::Real velocity1 = Ogre::Math::Abs((vp1->direction).dotProduct(n));
					Ogre::Real velocity2 = Ogre::Math::Abs((vp2->direction).dotProduct(n));
	...
	{
		Collider::copyAttributesTo(affector);
