...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	BoxSet::BoxSet(const Ogre::String& name, unsigned int poolSize, bool externalData) :
    ...
    //-----------------------------------------------------------------------
	Box* BoxSet::createBox(const Ogre::Vector3& position)
    {
		if(mFreeBoxes.empty())
		{
			if(mAutoExtendPool)
		...
		// Merge
		Ogre::Real adjust = std::max(mDefaultWidth, mDefaultHeight);
		adjust = std::max(adjust, mDefaultDepth);
        Ogre::Vector3 vecAdjust(adjust, adjust, adjust);
		Ogre::Vector3 newMin = position - vecAdjust;
		Ogre::Vector3 newMax = position + vecAdjust;

        mAABB.merge(newMin);
        mAABB.merge(newMax);

		Ogre::Real sqlen = std::max(newMin.squaredLength(), newMax.squaredLength());
		mBoundingRadius = std::max(mBoundingRadius, Ogre::Math::Sqrt(sqlen));

        return newBox;
	}
	//-----------------------------------------------------------------------
	Box* BoxSet::createBox(Ogre::Real x, Ogre::Real y, Ogre::Real z)
	{
		return createBox(Ogre::Vector3( x, y, z ));
	}
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	void BoxSet::setDefaultDimensions(Ogre::Real width, Ogre::Real height, Ogre::Real depth)
	{
		mDefaultWidth = width;
	...
	//-----------------------------------------------------------------------
	void BoxSet::setDefaultWidth(Ogre::Real width)
	{
		mDefaultWidth = width;
		mDefaultHalfWidth = 0.5 * width;
		_genDefaultCorners();
	...
	//-----------------------------------------------------------------------
	Ogre::Real BoxSet::getDefaultWidth(void) const
	{
		return mDefaultWidth;
	}
	//-----------------------------------------------------------------------
	void BoxSet::setDefaultHeight(Ogre::Real height)
	{
		mDefaultHeight = height;
		mDefaultHalfHeight = 0.5 * height;
		_genDefaultCorners();
	...
	//-----------------------------------------------------------------------
	Ogre::Real BoxSet::getDefaultHeight(void) const
	{
		return mDefaultHeight;
	}
	//-----------------------------------------------------------------------
	void BoxSet::setDefaultDepth(Ogre::Real depth)
	{
		mDefaultDepth = depth;
		mDefaultHalfDepth = 0.5 * depth;
		_genDefaultCorners();
	...
	//-----------------------------------------------------------------------
	Ogre::Real BoxSet::getDefaultDepth(void) const
	{
	...
	//-----------------------------------------------------------------------
	void BoxSet::setBounds(const Ogre::AxisAlignedBox& box, Ogre::Real radius)
	{
		mAABB = box;
    ...
    void BoxSet::_updateBounds(void)
    {
        ...
        else
        {
            ...
            {
                const Ogre::Vector3& pos = (*i)->getPosition();
                min.makeFloor(pos);
                max.makeCeil(pos);
	...
	void BoxSet::_updateRenderQueue(Ogre::RenderQueue* queue)
    {
		if (!mExternalData)
		{
			...
			ActiveBoxList::iterator it;
			for(it = mActiveBoxes.begin(); it != mActiveBoxes.end(); ++it)
			{
				injectBox(*(*it));
			}
			endBoxes();
	...
	//-----------------------------------------------------------------------
	bool BoxSet::boxVisible(Ogre::Camera* cam, const Box& box)
	{
		...
		{
			Ogre::Real max = std::max(box.mWidth, box.mHeight);
			max = std::max(box.mDepth, max);
			sph.setRadius(max);
		}
		else
		{
			Ogre::Real max = std::max(mDefaultWidth, mDefaultHeight);
	...
	//-----------------------------------------------------------------------
	const Ogre::String& BoxSet::getMovableType(void) const
	{
	...
	{
		mTextCoord [0] = Ogre::Vector2(0.0, 0.0);
		mTextCoord [1] = Ogre::Vector2(0.0, 1.0);
		mTextCoord [2] = Ogre::Vector2(0.5, 0.0);
		mTextCoord [3] = Ogre::Vector2(0.5, 1.0);

		mTextCoord [4] = Ogre::Vector2(0.0, 0.33);
		mTextCoord [5] = Ogre::Vector2(0.0, 0.66);
		mTextCoord [6] = Ogre::Vector2(0.5, 0.33);
		mTextCoord [7] = Ogre::Vector2(0.5, 0.66);

		mTextCoord [8] = Ogre::Vector2(0.5, 0.33);
		mTextCoord [9] = Ogre::Vector2(1.0, 0.33);
		mTextCoord [10] = Ogre::Vector2(0.5, 0.66);
		mTextCoord [11] = Ogre::Vector2(1.0, 0.66);

		mTextCoord [12] = Ogre::Vector2(0.5, 0.0);
		mTextCoord [13] = Ogre::Vector2(1.0, 0.0);
		mTextCoord [14] = Ogre::Vector2(0.5, 1.0);
		mTextCoord [15] = Ogre::Vector2(1.0, 1.0);
	}
	//-----------------------------------------------------------------------
	Ogre::String BoxSetFactory::PU_FACTORY_TYPE_NAME = "BoxSet";
	//-----------------------------------------------------------------------
	const Ogre::String& BoxSetFactory::getType(void) const
	{
		return PU_FACTORY_TYPE_NAME;
	}
	//-----------------------------------------------------------------------
	Ogre::MovableObject* BoxSetFactory::createInstanceImpl(const Ogre::String& name, const Ogre::NameValuePairList* params)
	{
		...
		if (params != 0)
		{
			...
			if (ni != params->end())
			{
				poolSize = Ogre::StringConverter::parseUnsignedInt(ni->second);
			}
			ni = params->find("externalData");
			if (ni != params->end())
			{
				externalData = Ogre::StringConverter::parseBool(ni->second);
			}
		}

		if (poolSize > 0)
		{
			return OGRE_NEW BoxSet(name, poolSize, externalData);
		}
		else
		{
			return OGRE_NEW BoxSet(name);
		}
	}
	//-----------------------------------------------------------------------
