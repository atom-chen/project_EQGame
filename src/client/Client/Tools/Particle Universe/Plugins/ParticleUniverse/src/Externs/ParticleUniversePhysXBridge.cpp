...
#include "ParticleUniversePrerequisites.h"
	...
	template<> ParticleUniverse::PhysXBridge* Ogre::Singleton<ParticleUniverse::PhysXBridge>::ms_Singleton = 0;
	...
	namespace ParticleUniverse
	{
		...
		//-----------------------------------------------------------------------
		//-----------------------------------------------------------------------
		class PhysXBridgeUpdateValue : public Ogre::ControllerValue<Ogre::Real>
		{
			protected:
				PhysXBridge* mTarget;
		...
		//-----------------------------------------------------------------------
		bool PhysXBridge::initNx(Ogre::Real gravityY)
		{
			if (!mPhysicsSDK)
			{
				...
				if(!mPhysicsSDK) 
				{
					OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR, "PU: Cannot initialise the PhysX SDK.", "PhysXBridge::initNx");
				}
			...
			if (!mScene)
			{
				...
				if(!mScene)
				{
					OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR, "PU: Cannot create a PhysX Scene.", "PhysXBridge::initNx");
				}
			...
			Ogre::ControllerValueRealPtr physXBridgeUpdateValue(OGRE_NEW PhysXBridgeUpdateValue(this));
			mTimeController = controllerManager.createFrameTimePassthroughController(physXBridgeUpdateValue);

			return true;
		}
		//-----------------------------------------------------------------------
		void PhysXBridge::_update(Ogre::Real timeElapsed)
		{
			// Simulate the PhysX scene every x seconds
			mTimeSinceLastSimulation += timeElapsed;
			if (mTimeSinceLastSimulation > SIMULATION_INTERVAL)
			{
		...
		//-----------------------------------------------------------------------
		void PhysXBridge::synchronize(Ogre::Real timeElapsed)
		{
		...
		bool PhysXBridge::onContactNotify(NxContactPair& pair, NxU32 events, NxVec3 contactPoint)
		{
			...
			*/
			...
			{
				if (isRegisteredActorCollisionGroup(pair.actors[0]->getGroup()))
				{
			...
			{
				if (isRegisteredActorCollisionGroup(pair.actors[1]->getGroup()))
				{
					// First check whether it is a particle
		...
		//-----------------------------------------------------------------------
		bool PhysXBridge::isRegisteredActorCollisionGroup(Ogre::uint16 actorCollisionGroup)
		{
			std::vector<PhysXExtern*>::iterator it;
			std::vector<PhysXExtern*>::iterator itEnd = mExternList.end();
			for (it = mExternList.begin(); it != itEnd; ++it)
			{
				if (actorCollisionGroup == (*it)->getActorGroup())
				{
					return true;
				}
			}
