...
{
	// Constants
	const Ogre::uint OnCountObserver::DEFAULT_THRESHOLD = 0;
	...
	//-----------------------------------------------------------------------
	bool OnCountObserver::_observe (ParticleTechnique* particleTechnique, Particle* particle, Ogre::Real timeElapsed)
	{
		if (!particle)
			return false;
