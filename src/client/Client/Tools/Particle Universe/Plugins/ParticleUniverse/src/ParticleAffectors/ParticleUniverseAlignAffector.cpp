...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	void AlignAffector::_affect(ParticleTechnique* particleTechnique, Particle* particle, Ogre::Real timeElapsed)
	{
		if (particle->particleType == Particle::PT_VISUAL)
		{
			// Set the orientation towards the previous particle, but rotation is undetermined.
			VisualParticle* visualParticle = visualParticle = static_cast<VisualParticle*>(particle);

			// Get difference
			Ogre::Vector3 diff = (mPreviousParticle->position - particle->position);
			if (mResize)
			{
