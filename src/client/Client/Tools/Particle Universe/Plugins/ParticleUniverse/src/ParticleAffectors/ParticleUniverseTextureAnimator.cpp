...
{
	// Constants
	const Ogre::Real TextureAnimator::DEFAULT_TIME_STEP = 0.0f;
	const Ogre::uint16 TextureAnimator::DEFAULT_TEXCOORDS_START = 0;
	const Ogre::uint16 TextureAnimator::DEFAULT_TEXCOORDS_END = 0;
	...
	//-----------------------------------------------------------------------
	Ogre::Real TextureAnimator::getAnimationTimeStep(void) const
	{
		return mAnimationTimeStep;
	}
	//-----------------------------------------------------------------------
	void TextureAnimator::setAnimationTimeStep(Ogre::Real animationTimeStep)
	{
	...
	//-----------------------------------------------------------------------
	Ogre::uint16 TextureAnimator::getTextureCoordsStart(void) const
	{
		return mTextureCoordsStart;
	}
	//-----------------------------------------------------------------------
	void TextureAnimator::setTextureCoordsStart(Ogre::uint16 textureCoordsStart)
	{
		mTextureCoordsStart = textureCoordsStart;
	}
	//-----------------------------------------------------------------------
	Ogre::uint16 TextureAnimator::getTextureCoordsEnd(void) const
	{
		return mTextureCoordsEnd;
	}
	//-----------------------------------------------------------------------
	void TextureAnimator::setTextureCoordsEnd(Ogre::uint16 textureCoordsEnd)
	{
	...
	void TextureAnimator::_initParticleForEmission(Particle* particle)
	{
		...
		if (mStartRandom)
		{
			visualParticle->textureCoordsCurrent = Ogre::Math::RangeRandom(mTextureCoordsStart, mTextureCoordsEnd);
		}
		else
		{
	...
	//-----------------------------------------------------------------------
	void TextureAnimator::_preProcessParticles(ParticleTechnique* particleTechnique, Ogre::Real timeElapsed)
	{
	...
	//-----------------------------------------------------------------------
	void TextureAnimator::_affect(ParticleTechnique* particleTechnique, Particle* particle, Ogre::Real timeElapsed)
	{
		// Only continue if the particle is a visual particle
		if (!particle->particleType == Particle::PT_VISUAL)
			return;
