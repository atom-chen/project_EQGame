...
void EditorFrame::Setup()
{
	...
	//g_msgText = new wxStaticText( this, wxID_STATIC, _("Static text"), wxDefaultPosition, wxDefaultSize, 0 );
}
...
void EditorFrame::UpdateLockedSelectionIcon(const EditorDocument* const document) const
{
    ...
    // Obtain toolbar
    wxToolBar* const mToolBar = this->GetToolBar();

    // Is the current user selection locked?
    if(document->GetLockedSelection())
    {
        // Selection locked, so change to locked icon
        mToolBar->SetToolNormalBitmap(ID_LOCK_SELECTION, wxBitmap (lock_xpm));
        mToolBar->SetToolShortHelp(ID_LOCK_SELECTION, wxT("Unlock Selection"));
    }
    else
    {
        // Selection unlocked, so change to unlocked icon
        mToolBar->SetToolNormalBitmap(ID_LOCK_SELECTION, wxBitmap (unlock_xpm));
        mToolBar->SetToolShortHelp(ID_LOCK_SELECTION, wxT("Lock Selection"));
    }
}
