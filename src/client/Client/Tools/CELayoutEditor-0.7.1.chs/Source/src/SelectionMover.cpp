...
void SelectionMover::UpdateWindowAreas(float left, float top, float right, float bottom)
{
    ...
    {   // Deny when it is blocked
        ...
        if (autoMovement)
        {
			...
			/*CEGUI::UDim w = boxIt->GetWindow()->getWidth();
			CEGUI::UDim h = boxIt->GetWindow()->getHeight();
			CEGUI::UVector2 p = boxIt->GetWindow()->getPosition();

			wxString txt;
			txt.Printf(wxT("P((%f,%f),(%f,%f)),W(%f,%f),H(%f,%f)"), p.d_x.d_offset, p.d_x.d_scale, p.d_y.d_offset, p.d_y.d_scale, w.d_offset, w.d_scale, h.d_offset, h.d_scale);
			g_msgText->SetLabel(txt);*/
        }
        else 
        {
            // Scale movement
...
void SelectionMover::UpdateWindowPosition( float xOffset, float yOffset )
{
	// Validations
	wxASSERT_MSG(m_selection != NULL, wxT("Selection member is NULL"));
	...
	{   // Deny when it is blocked
		...
		// Handy pointer
		CEGUI::Window* window = boxIt->GetWindow();

		CEGUI::UVector2 pt(window->getPosition());

		pt.d_x.d_offset += xOffset;
		pt.d_y.d_offset += yOffset;
