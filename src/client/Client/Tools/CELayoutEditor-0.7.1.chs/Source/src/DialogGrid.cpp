...
wxDialog(parent, wxID_ANY, wxT("网络参数设置"), wxDefaultPosition, wxSize(300, 255))
{
...
void DialogGrid::Setup()
{
    ...
    wxStaticBox* const mainGroup = new wxStaticBox(this, wxID_ANY, wxT("设置参数："));
    m_visible = new wxCheckBox(this, wxID_ANY, wxT("显示性") );

    // Snap options
    const wxString options[] = {wxT("捕捉 X轴"), wxT("捕捉 Y轴"), wxT("都捕捉")};
    m_snapModeChoice = new wxRadioBox(this, wxID_ANY, wxT("捕捉模式："), wxDefaultPosition, wxDefaultSize, 3, options);

    wxStaticText* const size = new wxStaticText(this, wxID_ANY, wxT("格子大小（像素）") );
    // Validate for positive number
    m_size = new wxTextCtrl(this, wxID_ANY);

    wxButton* const ok = new wxButton(this, ID_OK_BUTTON, wxT("确定") );
    wxButton* const cancel = new wxButton(this, ID_CANCEL_BUTTON, wxT("取消") );
