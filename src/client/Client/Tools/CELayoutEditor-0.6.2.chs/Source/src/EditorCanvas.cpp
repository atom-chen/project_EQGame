...
EVT_ERASE_BACKGROUND	(EditorCanvas::OnErase)
...
void EditorCanvas::DrawGrid() const
{
    ...
    if (gridVisible && step > 1)
    {
        // Prepare the loop
        int stepsX, stepsY;
        if (mCurrentHeight % step == 0)
            stepsY = mCurrentHeight / step;
        else
            stepsY = (mCurrentHeight / step) + 1;
        if (mCurrentWidth % step == 0)
            stepsX = mCurrentWidth / step;
        else
            stepsX = (mCurrentWidth / step) + 1;
        ...
        {
            // Same Y value
            startY = endY = 1.0f - ( (wx_static_cast(GLfloat, i * step) / wx_static_cast(GLfloat, mCurrentHeight-8.0f)) * 2.0f);
        ...
        {
            // Same X value
            startX = endX = -1.0f + ( (wx_static_cast(GLfloat, i * step) / wx_static_cast(GLfloat, mCurrentWidth-4.0f)) * 2.0f);
            glVertex2f(startX, startY);
            glVertex2f(endX, endY);
        }
...
void EditorCanvas::DrawResizers() const
{
    ...
    for (Selection::Boxes::iterator boxIt = boxes.begin(); boxIt != boxes.end(); ++boxIt)
    {
        ...
        topLeftX = -1.0f + ( (wx_static_cast(GLfloat, winRc.x) / mCurrentWidth) * 2.0f);
        ...
        bottomRightY = 1.0f - ( (wx_static_cast(GLfloat, winRc.GetBottom()) / mCurrentHeight) * 2.0f);
...
{
	if(NULL == CEGUI::System::getSingletonPtr())
		return;
...
//------------------------------------------------------------------------
void EditorCanvas::OnErase(wxEraseEvent& WXUNUSED(event))
{
    // Draw set background.. in progress
}
...
void EditorCanvas::OnKeyDown(wxKeyEvent& event)
{
    ...
    {
		if(0 == mKeydownCnt)
			((EditorDocument *)m_view->GetDocument())->GetSelection()->SaveMoveableBoxNRects();
		mKeydownCnt ++;
...
void EditorCanvas::OnKeyUp(wxKeyEvent& event)
{
	...
	((EditorDocument *)m_view->GetDocument())->PushActMoveUndoItem();
