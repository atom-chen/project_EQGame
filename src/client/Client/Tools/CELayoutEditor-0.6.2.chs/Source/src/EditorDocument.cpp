...
bool EditorDocument::OnNewDocument () 
{
	...
	wxMessageDialog dlg(NULL, wxT("是否要增加一个全屏的根窗体？"), wxT("增加默认根窗体？"),
		wxYES_NO);
...
CEGUI::Window* EditorDocument::AddWindow (CEGUI::Window* aParentWindow, const CEGUI::String& aType, CEGUI::String aName) 
{
    ...
    {   
        ...
        EditorDocument::Modify(true);
		PushActAddUndoItem(aParentWindow, newWindow);
        return newWindow;
    }
...
{   // Only process when a file is given
    ...
    {   // Remove current imageset. This call just silently bails out when not found.
		CEGUI::ImagesetManager::getSingleton().destroyImageset("BackgroundImage");
        ...
        {   	
            ...
            {   // Okay, behold... because ImageSets get loaded from the predefined resource group,
                ...
                (void)CEGUI::ImagesetManager::getSingleton().createImagesetFromImageFile("BackgroundImage", StringHelper::ToCEGUIString(file));
...
void EditorDocument::HandleMouseMoved (int aMouseX, int aMouseY) 
{
    ...
    if (m_mousePressed) 
    {
        ...
        {	
			...
			mover.UpdateWindowPosition(pixelDeltaX, pixelDeltaY);
            bChanges = true;
        }
    ...
    {	// Mouse not pressed; find hovered resize point or window
        if (m_activeLayout) 
        {
			...
			{
				CEGUI::Rect pixelRc = m_activeLayout->getPixelRect();
...
void EditorDocument::HandleMousePressed (const int /*WXUNUSED*/(aMouseX), const int /*WXUNUSED*/(aMouseY), const bool aMustReset) 
{
	...
	if(false == m_mousePressed)
	{
		...
		m_selection.SaveMoveableBoxNRects();
	}
}
...
void EditorDocument::HandleMouseReleased () 
{
	...
	if(m_xMDown != m_mouseX || m_yMDown != m_mouseY)
	{
		if(RESIZE_POINT_NONE != m_hoveredResizePoint)
			PushActResizeUndoItem();
		else
			PushActMoveUndoItem();
	}
    m_mousePressed = false ;
    ...
    m_hoveredWindow = NULL ;
    if (mustNotify)
		NotifyDraggedOrResized();
}

void EditorDocument::NotifyDraggedOrResized()
{
	for (size_t i = 0; i < m_observers.size(); ++i) 
	{
...
void EditorDocument::SelectAll()
{
	// Not when locked
    if(!m_lockedSelection)
	{	// Select the first layer of parent windows
		...
		while (!windowIt.isAtEnd())
		{
			...
			LOG_EXCEPTION("EditorDocument::SelectAll", "Error", "Unexpected standard exception while selecting all widgets", "Unexpected non-standard exception while selecting all widgets");
		}
		EditorDocument::Modify(true);
	}
} 
...
//////////////////////////////////////////////////////////////////////////
void EditorDocument::PushActMoveUndoItem()
{
	m_actManager.PushToUndo(new ActItemMove(m_selection.GetMoveableBoxNRects()));

}

void EditorDocument::PushActResizeUndoItem()
{
	m_actManager.PushToUndo(new ActItemResize(m_selection.GetMoveableBoxNRects()));

}

void EditorDocument::PushActDelUndoItem()
{
	ActItemDel *actDelObj = new ActItemDel();

	Selection::Boxes &bx = m_selection.GetMoveableBoxes();

	for(Selection::Boxes::iterator boxItr = bx.begin(); boxItr != bx.end(); ++boxItr)
	{
		CEGUI::Window *win = boxItr->GetWindow();

		actDelObj->GetWinClone().Clone(win);

		CEGUI::Window *parent = win->getParent();

		if(parent)
			actDelObj->GetWinClone().SetParent((CEGUI::String &)parent->getName());
	}

	m_actManager.PushToUndo(actDelObj);
}

void EditorDocument::PushActAddUndoItem( CEGUI::Window *parent, CEGUI::Window *window )
{
	ActItemNew *actNewObj = new ActItemNew();
	if(parent)
		actNewObj->GetWinClone().SetParent((CEGUI::String &)parent->getName());
	actNewObj->GetWinClone().Clone(window);

	m_actManager.PushToUndo(actNewObj);
}
//////////////////////////////////////////////////////////////////////////
