...
wxDialog(aParentDialog, wxID_ANY, wxT("新增窗体"), wxDefaultPosition, wxSize(350, 700))
{
...
void DialogAddWindow::Setup()
{
    ...
    // All static fields
    wxStaticText* const staticName = new wxStaticText(this, wxID_ANY, wxT("新窗体的名称："));
    wxStaticBox* const staticWidgets = new wxStaticBox(this, wxID_ANY, wxT("在当前父窗体下有效的窗体类型："));
    ...
    const wxString options[] = {wxT("显示所有窗体类型"), wxT("显示可被加入的窗体类型")};
    m_filterBox = new wxRadioBox(this, ID_FILTER_BOX, wxT("过滤器："), wxDefaultPosition, wxDefaultSize, 2, options);
    ...
    // it will be handled internally by wxWidgets wxSizer::Add()
    wxStaticText* const filterWarning = new wxStaticText(this, wxID_ANY, wxT("蓝色字体的为必需要一个父窗体的窗体类型"));
    ...
    (void)buttonSizer->Add(new wxButton(this, wxID_CANCEL, wxT("取消")), 0, wxEXPAND | wxALL, BORDER);
...
void DialogAddWindow::InitWidgetTree(const CEGUI::Window* const aParentWindow)
{
    ...
    COMPILE_TODO("Some of this code is repeated elsewhere. Uniformize!")
    ...
    for(; !falIt.isAtEnd() ; ++falIt)
    {
        ...
        if (Filter(widget, aParentWindow))
        {
            ...
            {	// Branch not available yet; add it.
                const wxTreeItemId node = m_widgetTree->AppendItem(rootNode, StringHelper::ToWXString(look));
