...
bool CPropertyGrid::SetItemEditable(HITEM item, bool bValue)
{
	CItem* pItem = FindItem(item);
	if(!pItem)
		return false;

	pItem->m_editable = bValue;

	return true;
}

bool CPropertyGrid::SetSectionEditable(HSECTION hs, bool bValue)
{
	vector<CSection>::iterator it = find(m_sections.begin(), m_sections.end(), hs);
	if (it == m_sections.end())
		return false;

	vector<CItem>::iterator it2;
	for(it2=it->m_items.begin(); it2!=it->m_items.end(); it2++)
		it2->m_editable = bValue;

	return true;
}

void CPropertyGrid::SetSectionCollapsed(HSECTION hs, bool bValue)
{
	CSection* pSection = FindSection(hs);
	if (pSection)
		pSection->m_collapsed = bValue;
}
