...
#include "SearchSystem/Delaunay.h"
...
void CCustomShape::buildPlanePoint()
{
	...
	SearchSystem::Polygon poly;
	for(int i = 0; i < iPointCount; ++i)
		poly.addVertex(SearchSystem::Vector2f(m_pointArray[i].x, m_pointArray[i].z));
	poly.CCW();

	SearchSystem::Delaunay d;
	const std::vector<SearchSystem::Triangle>& tri = d.createDelaunay(poly);

	for ( size_t i = 0; i < tri.size(); ++i )
	{
		for ( int j = 0; j < 3; ++j )
		{
			SearchSystem::Vector2f v2 = tri[i].getVertex(j);
			float y = 0;
			for (int k = 0; k < iPointCount; ++k)
			{
				if ( m_pointArray[k].x == v2.x && m_pointArray[k].z == v2.y )
				{
					y = m_pointArray[k].y;
					break;
				}
			...
			m_planePointArray.push_back(Ogre::Vector3(v2.x, y, v2.y));
		}
	}
#else
