...
{
	std::string name = "IntersectGridMaterial";
...
{
	mIntersectGridsNode = RootManager::getSingleton().getIndicatorRootSceneNode()->createChildSceneNode();
...
IntersectGrids::~IntersectGrids()
{
	...
	{
		mIntersectGridsNode->removeAndDestroyAllChildren();
		mIntersectGridsNode->getCreator()->destroySceneNode(mIntersectGridsNode);
	}
...
{
	static const std::string name = "IntersectGrids";
	return name;
}
...
IntersectGrids::_setHitPoint(Real x, Real y)
{
	GridSelection* selection = 
		static_cast<GridSelection *>(RootManager::getSingleton()._getSelection("GridSelection"));
...
// FlipDiagonalGrids                                                /////
//////////////////////////////////////////////////////////////////////////
FlipDiagonalGrids::FlipDiagonalGrids()
{
	intersectGrids->setGridColour( Ogre::ColourValue(1.0, 0.0, 0.0, 1.0) );
}

const std::string& FlipDiagonalGrids::getName(void) const
{
	static const std::string name = "FlipDiagonalGrids";
	return name;
}    
...
const std::string& DisableDecalGrids::getName(void) const
{
	static const std::string name = "DisableDecalGrids";
	return name;
}
...
void DisableDecalGrids::_createProjector(void)
{
	if (NULL == mProjector)
	{
		mProjectorNode = RootManager::getSingleton().getIndicatorRootSceneNode()->createChildSceneNode();

		mProjector = static_cast<Projector*>
			( SystemGcs::getSingleton().getSceneManager()->
...
void DisableDecalGrids::_destroyProjector(void)
{
	if (mProjector)
	{
		...
		SystemGcs::getSingleton().getSceneManager()->destroyMovableObject(mProjector);

		mProjector = NULL;

		if (mProjectorNode)
		{
			mProjectorNode->removeAndDestroyAllChildren();
			mProjectorNode->getCreator()->destroySceneNode(mProjectorNode);
		}
	}
}
...
void DisableDecalGrids::_setProjcetorSize(void)
{
	...
	size_t xBrushSize = RootManager::getSingleton().getBrushXSize() * EQSceneManager::getSingleton().getTerrainData()->mScale.x;
	size_t zBrushSize = RootManager::getSingleton().getBrushZSize() * EQSceneManager::getSingleton().getTerrainData()->mScale.z;

	mProjector->setOrthoWindowHeight(xBrushSize);
...
{
	mTotalGridSelection = new GridSelection();
	mTotalJunctionSelection = new JunctionSelection();
}
...
const std::string& TerrainSelectionGrids::getName(void) const
{
	static const std::string name = "TerrainSelectionGrids";
	return name;
}
...
void TerrainSelectionGrids::_setHitPoint(Real x, Real y)
{
	...
	// 计算出实际的相交点
	intersected = RootManager::getSingleton().getTerrainIntersects(left, top, firstPos);
	intersected = RootManager::getSingleton().getTerrainIntersects(right, bottom, lastPos);

	std::pair<int, int> minGrid = EQSceneManager::getSingleton().getTerrainData()->getGrid(firstPos.x, firstPos.z);
	std::pair<int, int> maxGrid = EQSceneManager::getSingleton().getTerrainData()->getGrid(lastPos.x, lastPos.z);
	...
	for ( int i=mini; i<maxi+1; ++i )
		...
		Ogre::Vector3 sceneNodePos = 
			EQSceneManager::getSingleton().getTerrainData()->_getPosition( (mMaxX-mMinX)/2+mMinX, (mMaxZ-mMinZ)/2+mMinZ);
...
void TerrainSelectionGrids::setCurrentGridSelection( bool multiSelecting )
{
	...
	mCurrentSelection = new GridSelection();
