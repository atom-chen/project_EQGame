...
#include "ModifyTerrainPaintOperator.h"
...
//-----------------------------------------------------------------------
const std::string& ModifyTerrainPaintOperator::getGroupName(void) const
{
	static const std::string groupName = "TerrainPaint";
	return groupName;
}
//-----------------------------------------------------------------------
std::string ModifyTerrainPaintOperator::getDescription(void) const
{
	// TODO:
	return "";
}
//-----------------------------------------------------------------------
std::string ModifyTerrainPaintOperator::getHelp(void) const
{
	// TODO
	return "";
}
...
void ModifyTerrainPaintOperator::apply(UndoRedoGridInfo Info::* undoRedo)
{
	if (!mInfos.empty())
	{
		for (InfoList::const_iterator i = mInfos.begin(); i != mInfos.end(); ++i)
		{
			...
			const TerrainData::GridInfo &nowInfo = EQSceneManager::getSingleton().getTerrainData()->getGridInfo(info.x, info.z);
			...
			{
				int layerOnePixmapId = EQSceneManager::getSingleton().getTerrainData()->_registerPixmap(
			...
			{
				int layerTwoPixmapId = EQSceneManager::getSingleton().getTerrainData()->_registerPixmap(
			...
			EQSceneManager::getSingleton().getTerrainData()->setGridInfo(info.x, info.z, newInfo);
		}
