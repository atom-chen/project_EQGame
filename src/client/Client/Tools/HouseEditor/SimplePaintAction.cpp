...
//-----------------------------------------------------------------------
SimplePaintAction* SimplePaintAction::create()
{
	return new SimplePaintAction();
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
const std::string& SimplePaintAction::getName(void) const
{
	static const std::string name = "SimplePaintAction";
	return name;
}
//-----------------------------------------------------------------------
void SimplePaintAction::setParameter(const std::string& name, const std::string& value)
{
...
//-----------------------------------------------------------------------
std::string SimplePaintAction::getParameter(const std::string& name) const
{
	if (Ogre::StringUtil::startsWith(name, "%", false))
	{
		std::string parameter = name.substr(1);
...
//-----------------------------------------------------------------------
void SimplePaintAction::_buildHitIndicator(const Ogre::Vector2& pt)
{
	...
	if (!mbTexNormalSize)
	{
		int oldBrushXSize = RootManager::getSingleton().getBrushXSize();
		int oldBrushZSize = RootManager::getSingleton().getBrushZSize();

		// 临时设置画刷大小
		RootManager::getSingleton().setBrushXSize(oldBrushXSize*mXBrushSize);
		RootManager::getSingleton().setBrushZSize(oldBrushZSize*mZBrushSize);
		...
		// 构建完区域后又设置回去
		RootManager::getSingleton().setBrushXSize(oldBrushXSize);
		RootManager::getSingleton().setBrushZSize(oldBrushZSize);
	}
	else
	{
		RootManager::getSingleton().getHitIndicator("IntersectGrids")->setHitPoint(pt);
		...
		for (GridSelection::GridMap::const_iterator it = grids.begin(); it != grids.end(); ++it)
		{
			...
			if (isTransparent)
			{
				...
				*/
				...
				std::string layerOneTexName = terrainData->mTextures[ terrainData->mPixmaps[info.layers[0].pixmapId-1].textureId ];
				assert ( !layerOneTexName.empty() );

				std::string layerOneBrushName = 
					RootManager::getSingleton().getTerrainPaintInfoContainer()->findBrushNameByTexName(layerOneTexName);				

				std::string layerTwoTexName = terrainData->mTextures[ terrainData->mPixmaps[pixmapId-1].textureId ];
				assert ( !layerTwoTexName.empty() );

				std::string layerTwoBrushName = 
					RootManager::getSingleton().getTerrainPaintInfoContainer()->findBrushNameByTexName(layerTwoTexName);
...
//-----------------------------------------------------------------------
void SimplePaintAction::_onMove(const Ogre::Vector2& pt)
{
	mHintModified->apply();
	mHintModified->reset();
...
//-----------------------------------------------------------------------
void SimplePaintAction::_onBegin(const Ogre::Vector2& pt)
{
	mHintModified->apply();
	mHintModified->reset();
...
//-----------------------------------------------------------------------
void SimplePaintAction::_onDrag(const Ogre::Vector2& pt)
{
	_buildHitIndicator(pt);
	_doPaint(mModifiedGrids);
}
//-----------------------------------------------------------------------
void SimplePaintAction::_onEnd(const Ogre::Vector2& pt, bool canceled)
{
	/*if (canceled)
	{
