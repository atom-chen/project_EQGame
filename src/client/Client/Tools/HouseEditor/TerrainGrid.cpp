...
TerrainGridRenderable::init(int xStart,int zStart, int xEnd, int zEnd)
{
	...
	terrData = EQSceneManager::getSingleton().getTerrainData();
...
: mActive(false) 
{
	...
	mSceneMgr = sceneMgr;
	SceneListenerManager::getSingleton().addSceneListener(this);

	mBaseSceneNode = RootManager::getSingleton().getIndicatorRootSceneNode()->createChildSceneNode();
	mTerrData = EQSceneManager::getSingleton().getTerrainData();
}

TerrainGrid::~TerrainGrid()
{
	SceneListenerManager::getSingleton().removeSceneListener(this);

	destroyRenderable();

	if (mBaseSceneNode)
	{
		mBaseSceneNode->removeAndDestroyAllChildren();
