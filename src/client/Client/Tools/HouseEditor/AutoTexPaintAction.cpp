...
//-----------------------------------------------------------------------
AutoTexPaintAction::BrushMap AutoTexPaintAction::mBrushMap;
//-----------------------------------------------------------------------
void AutoTexPaintAction::clearBrushMap()
{
...
//-----------------------------------------------------------------------
AutoTexPaintAction* AutoTexPaintAction::create()
{
	return new AutoTexPaintAction();
}
//-----------------------------------------------------------------------
...
{
	mCurrentInnerGrids = new GridSelection();
	mCurrentOuterGrids = new GridSelection();
...
//-----------------------------------------------------------------------
const std::string& AutoTexPaintAction::getName(void) const
{
	static const std::string name = "AutoTexPaintAction";
	return name;
}


void AutoTexPaintAction::setParameter(const std::string& name, const std::string& value)
{
	if (Ogre::StringUtil::startsWith(name, "%", false))
	{
		std::string parameter = name.substr(1);
		...
		else if ( parameter == "TextureName" )
		{
			...
			if ( mInitCurrentBrush == false )
			{
				...
				else
				{
					...
					const TextureInfo &currentPaintInfo = RootManager::getSingleton().getTextureInfo(mCurrentBrushName, value);

					PaintPixmap pixmap(currentPaintInfo.ownerTextureName);

					Ogre::Image *ownerImage = RootManager::getSingleton().getPreviewImage(currentPaintInfo.ownerTextureName);
					...
					else if ( currentPaintInfo.textureType == InnerTransTex )
					{
						// 啥都不干 Y3
						mInitCurrentBrush = false;
					}
...
//-----------------------------------------------------------------------
std::string AutoTexPaintAction::getParameter(const std::string& name) const
{
...
//-----------------------------------------------------------------------
void AutoTexPaintAction::_buildHitIndicator(const Ogre::Vector2& pt)
{
	...
	// 选取不同的画刷，构建不同的选择区域
	RootManager::getSingleton()._buildSelection(mCurrentInnerGrids, position.x, position.z);

	buildBoxEdgeSelection(*mCurrentOuterGrids,position.x,position.z,RootManager::getSingleton().getBrushXSize(), RootManager::getSingleton().getBrushZSize());
	//refresh();
}
//-----------------------------------------------------------------------
void AutoTexPaintAction::_doPaint(GridSelection* modified)
{
	...
	{ 
		...
		int maxx = mCurrentBoxSelectionMinX + RootManager::getSingleton().getBrushXSize()+1;
		int maxz = mCurrentBoxSelectionMinZ + RootManager::getSingleton().getBrushZSize()+1;
		...
		{     
			...
			// LEFT
				...
				assert(info.layers[0].pixmapId <= EQSceneManager::getSingleton().getTerrainData()->mPixmaps.size());

				if (info.layers[0].pixmapId > EQSceneManager::getSingleton().getTerrainData()->mPixmaps.size())
				{
					OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR,
				...
				assert(info.layers[1].pixmapId <= EQSceneManager::getSingleton().getTerrainData()->mPixmaps.size());

				if (info.layers[1].pixmapId > EQSceneManager::getSingleton().getTerrainData()->mPixmaps.size())
				{
					OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR,
				...
				if (EQSceneManager::getSingleton().getTerrainData()->mPixmaps.size() >= 256)
				{
					;
				}
...
//-----------------------------------------------------------------------
void AutoTexPaintAction::_onMove(const Ogre::Vector2& pt)
{      
	mHintModified->apply();
	mHintModified->reset();
...
//-----------------------------------------------------------------------
void AutoTexPaintAction::_onBegin(const Ogre::Vector2& pt)
{
	mHintModified->apply();
	mHintModified->reset();
...
//-----------------------------------------------------------------------
void AutoTexPaintAction::_onDrag(const Ogre::Vector2& pt)
{
	_buildHitIndicator(pt);
	_doPaint(mModifiedGrids);
}
//-----------------------------------------------------------------------
void AutoTexPaintAction::_onEnd(const Ogre::Vector2& pt, bool canceled)
{
	/*if (canceled)
	{
...
#define error 0.000001
		...
		IsCaseInsensitiveEqual(const std::string& value, Real left, Real top) : value(value), left(left), top(top) {}
...
bool AutoTexPaintAction::isTexInCurrentGroup( ushort id )
{
	...
	{
		TerrainData::Pixmap pixmap = EQSceneManager::getSingleton().getTerrainData()->mPixmaps[id-1];
		std::string texName = EQSceneManager::getSingleton().getTerrainData()->mTextures[ pixmap.textureId ];
