...
// 设置材质
void CPointInfo::SetMaterial(std::string strMaterialName)
{
	if(m_pEntity)
	{
...
unsigned long CPointInfo::CreateEntity()
{
	...
	if(/*(m_pSceneManipulator)&&(m_pSceneManipulator->getSceneManager())*/SystemGcs::getSingletonPtr())
	{
		char Namebuf[256];
		memset(Namebuf, 0, sizeof(Namebuf));
		sprintf(Namebuf, "Region%d_Point%d_%d", m_ulParentId, m_ulPointId, iCount);

		Ogre::MeshPtr sphereMesh = createSphereMesh(SystemGcs::getSingleton().getSceneManager(), HIT_SPHERE_RADIUS, 16, 16);
		m_pEntity =SystemGcs::getSingleton().getSceneManager()->createEntity(Namebuf, sphereMesh->getName());
...
unsigned long CUnreachRegion::AddNewPoint1(Ogre::Vector3 position)
{
	...
	m_ulCurMaxPointId++;
	unsigned long ulPointId = m_ulCurMaxPointId;
	CPointInfo* pPointInfo = new CPointInfo(position, m_pSceneNode, ulPointId, m_ulId);
	if(pPointInfo)
	{
...
// 设置区域材质
void CUnreachRegion::SetRegionMaterial(const std::string& strMatrerial)
{
	POINT_MAP::iterator it;
	POINT_MAP::iterator itEnd;
...
bool CUnreachRegion::IsMouseSel(float x, float y)
{
	...
	pCamera   = GraphicsSystem::getSingleton().getCamera();
...
CUnreachRegion::CUnreachRegion(unsigned long ulId, Ogre::SceneNode* pParentSceneNode)
: m_ulId(ulId)				
, m_ulCurMaxPointId(0)
, m_ulCurMaxLineId(0)
