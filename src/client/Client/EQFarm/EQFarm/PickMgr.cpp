...
void PickMgr::movein()
{
	...
	else if (po->getClassName() == "FarmObject")
	{
		FarmObject* farmObj = static_cast<FarmObject*>(po);
		sFarmObjEvent.onMoveinFarmObj(farmObj,mHighLight);
	}
...
void PickMgr::moveout()
{
	...
	{
		FarmObject* farmObj = static_cast<FarmObject*>(po);
		sFarmObjEvent.onMoveoutFarmObj(farmObj);
	}
...
bool PickMgr::click()
{
	...
	{
		FarmObject* farmObj = static_cast<FarmObject*>(po);
		sFarmObjEvent.onClickFarmObj(farmObj);
	}
