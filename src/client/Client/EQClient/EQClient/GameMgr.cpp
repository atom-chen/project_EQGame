...
bool GameMgr::init(HINSTANCE hInstance)
{
	{//单件体都在这里new出来，这里从理论上来讲没有顺序要球
	 //但最好也按比较合理的方式
		new OgreRootEx("", "", GetGameUserDir() + "EQClient.Ogre.log");
...
// 		new BodyActionMgr;
		...
		new MicroBlogManager;	// (2011/4/18 add by hezhr)
		new SmallMapManager;	// 小地图(2011/5/9 add by hezhr)
		//  礼物系统
		new GiftSystem;
		//  礼物记录管理器
		new GiftLogManager;
	 ...
	 */
		...
		VariableSystem::getSingleton().Initial();
		...
		try
		{
			...
			CGUIManager::getSingleton().Initialize(GetGameUserDir() + "EQClient.CEGUI.log");
...
// 	delete BodyActionMgr::getSingletonPtr();
	...
	delete MicroBlogManager::getSingletonPtr();	// (2011/4/18 add by hezhr)
	delete SmallMapManager::getSingletonPtr();	// (2011/5/9 add by hezhr)

	delete GiftSystem::getSingletonPtr();
	delete GiftLogManager::getSingletonPtr();
	delete MagicBoxSystem::getSingletonPtr();
