...
void
TerrainData::_loadHeightmap(const std::string& filename, const std::string& type, const std::string& groupName)
{
    // Allocate buffers
    ...
    else if (type == "image")
    {
        ...
        {
            std::string err = "Error: Invalid heightmap size : " +
...
void
TerrainData::_saveHeightmap(const std::string& filename) const
{
...
void
TerrainData::_loadLightmap(const std::string& filename, const std::string& type, const std::string& groupName)
{
...
#undef new
		mLightmapImage = OGRE_NEW Ogre::Image;
#pragma pop_macro("new")
        try
        {
            mLightmapImage->load(Ogre::ResourceGroupManager::getSingleton().openResource(filename,grp));
        }
...
void
TerrainData::_saveLightmap(const std::string& filename) const
{
    if (!mLightmapImage)
    {
        return;
...
void
TerrainData::_loadGridInfo(const std::string& filename, const std::string& groupName)
{
...
void
TerrainData::_saveGridInfo(const std::string& filename) const
{
    FileWriter writer(filename);
	...
	bool gridInfoExist = _findGridInfoCopy(filename, gridInfoCopysIndex);
	if (gridInfoExist)
		mGridInfoArrayCopys.erase(mGridInfoArrayCopys.begin()+gridInfoCopysIndex);
}

bool 
TerrainData::_findGridInfoCopy(const std::string& fileName, size_t& index) const
{
    for (size_t i = 0; i < mGridInfoArrayCopys.size(); ++i)
    {
		if ( Ogre::StringUtil::endsWith(fileName, mGridInfoArrayCopys[i].fileName, false) )
        {
            index = i;
            return true;
        }
...
void
TerrainData::load(const std::string& filename, const std::string& groupName, bool loadLightmap)
{
    // Clear old data
    reset();
    ...
    _loadBaseInfo(filename, groupName);
...
void
TerrainData::save(const std::string& filename, bool preservingNames) const
{
...
namespace {
    struct IsCaseInsensitiveEqual
    {
        ...
        IsCaseInsensitiveEqual(const std::string& value) : value(value) {}
    ...
    static void normalisePath(std::string& name)
    {
...
int
TerrainData::_registerPixmap(const std::string& textureName, Real left, Real top, Real right, Real bottom)
{
    size_t pixmapId;
...
TerrainData::_optimize(void)
{
...

    {
        ...
        {
            const std::string& texture = it->first;
