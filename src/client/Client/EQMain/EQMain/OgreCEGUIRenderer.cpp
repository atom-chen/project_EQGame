...
#undef new
	d_render_op.vertexData = OGRE_NEW VertexData;
...
void OgreCEGUIRenderer::doRender(void)
{
	...
	if (d_render_sys->_getViewport()->getOverlaysEnabled() && !d_quadlist.empty())
	{
        ...
        while(i != d_quadlist.end())
        {
			...
			//add by Y3 没用到时候就解除资源引用, 否则在资源回收那里会出现引用不正常而throw异常
...
void OgreCEGUIRenderer::constructor_impl(Ogre::RenderWindow* window, Ogre::uint8 queue_id, bool post_queue, uint max_quads)
{
	...
	//-------------------------------------------
	d_isEnableRecycleTexture = false;
	d_recycleTextureInterval = 10*60*1000;	//10分钟
	d_recycleTextureListener = NULL;
}
...
/*************************************************************************
enableRecycleTextures
*************************************************************************/
void OgreCEGUIRenderer::enableRecycleTexture(bool e)
{
	d_isEnableRecycleTexture = e;
}

/*************************************************************************
enableRecycleTextures
*************************************************************************/
void OgreCEGUIRenderer::recycleTextures()
{
	if (!d_isEnableRecycleTexture)
		return;


	long ct = clock();

	System::getSingleton().signalRedraw();	//下次需要重新渲染
	clearRenderList();	//清除渲染列表的引用

	size_t allTexSize = 0;
	size_t recycleTexSize = 0;
	size_t recycleTexCount = 0;
	std::list<OgreCEGUITexture*>::iterator it;
	for (it = d_texturelist.begin(); it != d_texturelist.end(); ++it)
	{
		OgreCEGUITexture* tex = *it;
		allTexSize += tex->getTextureSize();
		if (ct - tex->getLastVisitTexTime() > d_recycleTextureInterval)		//如果大于时间间隔, 回收资源
		{
			size_t size = 0;
			bool ok = tex->recycleTexture(size);
			if (ok)
			{
				recycleTexSize += size;
				recycleTexCount++;
	...
	long useTime = clock() - ct;

	if (d_recycleTextureListener)
		d_recycleTextureListener->recycle(useTime, d_texturelist.size(), allTexSize, recycleTexCount, recycleTexSize);
