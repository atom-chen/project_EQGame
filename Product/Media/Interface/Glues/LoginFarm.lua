...
g_WinMgr:getWindow("Login/Port"):setText("2223")
...
g_WinMgr:getWindow("Login/exit"):subscribeEvent("Clicked", "LoginWindows_OnExit")
g_WinMgr:getWindow("Login/register"):subscribeEvent("Clicked", "LoginWindows_OnRegister")
...
g_WinMgr:getWindow("MessageBox/OK"):subscribeEvent("Clicked", "LoginWindows_OnMessageBoxOk")
...
function LoginWindows_OnLogin(args)
	...
	local password	= g_WinMgr:getWindow("Login/UserPassw"):getText()
	...
	if false == EQGlobaFunc.VersionRequest(account, password, ip, tonumber(port)) then
		...
		g_sceneMgr:startLoading()
	end
end

function LoginWindows_OnExit()
	EQGlobaFunc.ExitClient()
end

function LoginWindows_OnRegister()
	EQGlobaFunc.OpenURL("http://my.35info.cn/Register/EReg.aspx?serviceid=10")
end
