...
-- 事件注册
g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):subscribeEvent("MouseEnter", "WallowTime_enterTouxiang")
g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):subscribeEvent("MouseLeave", "WallowTime_leaveTouxiang")
-- 提示内容
local m_content = 
{
...
function WallowTime_showHint(x, y)
	...
	g_mlbParse:getData(WallowTime_formatTime(m_time), m_onlineTime:getName())
	...
	g_mlbParse:getData(WallowTime_getTip(m_time), m_tip:getName())
	...
	g_MainUIWindows:addChildWindow(m_hint)
end
-----------------------------------------
-- 隐藏头像hint
function WallowTime_hideHint()
	g_MainUIWindows:removeChildWindow(m_hint)
	...
	if seconds >= 0 then	-- 玩家需要防沉迷
		...
		if WallowTime_checkTime(m_time) then
			...
			if m_time>3*m_hour and m_time<5*m_hour then	-- 在3-5个小时间
				m_temp, m_msgBoxName = MsgBox_createMsgBoxExA(g_MainUIWindows, 200, "消息提示", m_content[7], {}, "MsgBoxEx_close", "")
			end
			if m_time > 5*m_hour then	-- 大于5个小时
				m_temp, m_msgBoxName = MsgBox_createMsgBoxExA(g_MainUIWindows, 200, "消息提示", m_content[8], {}, "MsgBoxEx_close", "")		
			end
		end
...
function WallowTime_checkTime(seconds)
	...
	if nil ~= tip then
		...
		m_temp, m_msgBoxName = MsgBox_createMsgBoxExA(g_MainUIWindows, 200, "消息提示", tip, {}, "MsgBoxEx_close", "")
		return true
	end
	return false
end
...
function WallowTime_changeTouxiang(dif)
	if (EQGlobaFunc.GetTickCountByLua()-m_twinkleStart) >= dif then
		...
		if 1 == m_twinkleFlag then
			...
			g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):setProperty("ImageColours", "tl:FFF1D2D2 tr:FFF1D2D2 bl:FFF1D2D2 br:FFF1D2D2")
		elseif 2 == m_twinkleFlag then
			m_twinkleFlag = m_twinkleFlag + 1
			g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):setProperty("ImageColours", "tl:FFF1AFAF tr:FFF1AFAF bl:FFF1AFAF br:FFF1AFAF")
		elseif 3 == m_twinkleFlag then
			m_twinkleFlag = m_twinkleFlag + 1
			g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):setProperty("ImageColours", "tl:FFF19292 tr:FFF19292 bl:FFF19292 br:FFF19292")
		elseif 4 == m_twinkleFlag then
			m_twinkleFlag = m_twinkleFlag + 1
			g_WinMgr:getWindow("DFwnd_MainRoot/tip/touxiang6"):setProperty("ImageColours", "tl:FFF26A6A tr:FFF26A6A bl:FFF26A6A br:FFF26A6A")
