...
local m_content = 
{
	"您累计在线时间已满1个小时。",
	"您累计在线时间已满2个小时。",
	"您累计在线时间已满3个小时，请您下线休息，做适当身体运动。",
	"您已进入疲劳游戏时间，您的游戏收益将降为正常值的50%，为了您的健康，请尽快下线休息，做适当身体运动，合理安排学习生活。",
	...
	--if seconds >= 0 then	-- 玩家需要防沉迷
		...
		--else
			--if m_time>3*m_hour and m_time<5*m_hour then	-- 在3-5个小时间
				--m_temp, m_msgBoxName = MsgBox_createMsgBoxExA(g_FarmUI, 200, "消息提示", m_content[4], {}, "MsgBoxEx_close", "")
			--end
			--if m_time > 5*m_hour then	-- 大于5小时
				--m_temp, m_msgBoxName = MsgBox_createMsgBoxExA(g_FarmUI, 200, "消息提示", m_content[5], {}, "MsgBoxEx_close", "")		
			--end
		--end
...
function WallowTime_checkTime(seconds)
	...
	if 3.5*m_hour==seconds or 4*m_hour==seconds or 4.5*m_hour==seconds then	-- 3.5个小时,4个小时,4.5个小时
		tip = m_content[4]
	end
